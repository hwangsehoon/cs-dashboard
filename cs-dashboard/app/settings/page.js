...
export default function SettingsPage() {
  ...
  const [selectedBrand, setSelectedBrand] = useState('azacha');
  ...
  // 현재 브랜드의 채널 설정들
  const currentBrandConfigs = configs.filter(c => c.brand_id === selectedBrand);
  const brandInfo = brands.find(b => b.id === selectedBrand);
  const availableChannels = brandChannels[selectedBrand] || [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-8">
          <h3 className="text-amber-400 font-semibold mb-2">📌 시작하기 전에</h3>
          <ol className="text-sm text-amber-200/80 space-y-1 list-decimal list-inside">
            <li><a href="https://supabase.com" target="_blank" className="underline">Supabase</a>에서 무료 프로젝트를 생성하세요</li>
            <li>SQL Editor에서 <code className="bg-slate-700 px-1 rounded">supabase-schema.sql</code> 파일을 실행하세요</li>
            <li><code className="bg-slate-700 px-1 rounded">.env.local</code> 파일에 Supabase URL과 Key를 입력하세요</li>
            <li>서버를 재시작하면 데이터가 연동됩니다</li>
          </ol>
        </div>
